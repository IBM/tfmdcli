...
function pullSubStrData(arr, string, addDefaultBreaks) {
  ...
  let name = string.substr(0, firstOpenBrace).replace(/\s/g, "");
...
function formatSimpleTfvar(varName, varType, varDefault) {
  ...
  let varTypeNotOptional = varType.replace(/(optional\(|(?<=optional.+)\))/g, "")
