...
function pullSubStrData(arr, string, addDefaultBreaks) {
  // Before doing anything, remove any whitespace from both ends of the string
  string = string.trim();
  // Fing opening of block
  let firstOpenBrace = string.indexOf("{");
  // Get the name of the block without spaces and quotes
...
function makeExampleTfvar(entry, noDefault) {
  ...
  if (simpleTypes.indexOf(varType) !== -1) {
    ...
    if (varType.match(/^list\((string|bool|number)\)$/)) {
      ...
      // If the variable has a default, return default after formating
      return `${varName}=${varDefault.replace(/\s(?=([\w_\-]+\s=\s)|\}\]?)/g, "\n")}`;
